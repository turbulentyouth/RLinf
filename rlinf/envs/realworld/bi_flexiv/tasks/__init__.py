...
from rlinf.envs.realworld.bi_flexiv.bi_flexiv_env import BiFlexivDualEnv
from rlinf.envs.realworld.bi_flexiv.recorder import BiFlexivDualLeRobotRecorder
from rlinf.envs.realworld.common.wrappers import KeyboardEvalControlWrapper


def create_bi_flexiv_env(
    ...
    try:
        if recording and bool(recording.get("enabled", False)):
            ...
            recorder = BiFlexivDualLeRobotRecorder(
        ...
        # Recorder 构造失败时，BiFlexivDualEnv 已把双臂停在起始位；异常若直接
        # 逃出 EnvWorker.__init__，Ray 会回收 worker 进程而跳过 close()，
        # 机械臂保持在上电状态。先尽力断开（内含回零）再原样抛出。close()
        # 内部会记录自身的失败，这里不再重复打日志。
        try:
            env.close()
        except Exception:
            pass
    ...
    if recorder is not None and not bool(control.get("enabled", False)):
        recorder.close()
        raise ValueError(
            "Flexiv recording requires env.eval.episode_control.enabled=true."
        )
    return env
...
register(
    ...
    entry_point=("rlinf.envs.realworld.bi_flexiv.tasks:create_bi_flexiv_env"),
)
