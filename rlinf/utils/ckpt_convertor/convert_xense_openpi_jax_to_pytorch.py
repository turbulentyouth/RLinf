#!/usr/bin/env python3
"""Convert an xense-openpi JAX checkpoint to an OpenPI PyTorch checkpoint.

Loads a JAX (orbax) Pi0/Pi05 checkpoint through
``openpi.models.model.restore_params``, translates the PaliGemma / Gemma-expert /
projection parameters into the OpenPI PyTorch ``paligemma_with_expert.*`` layout,
and writes ``model.safetensors`` together with ``config.json``. The
``norm_stats.json`` asset is copied verbatim into
``assets/<asset-id>/norm_stats.json``.

The target Pi0/Pi05 architecture is resolved from ``config_name`` through
``rlinf.models.embodiment.openpi.dataconfig.get_openpi_config`` (for example
``pi05_bi_flexiv``), matching SFT and eval. The norm-stats asset is copied by
default to ``assets/<asset-id>/norm_stats.json`` unless ``--no-copy-norm-stats``
is passed.

Usage::

    .venv/bin/python rlinf/utils/ckpt_convertor/convert_xense_openpi_jax_to_pytorch.py \
        /path/to/jax_checkpoint \
        pi05_bi_flexiv \
        --output-path /path/to/output \
        --precision bfloat16 \
        --norm-stats /path/to/norm_stats.json

    # Inspect parameter keys only:
    .venv/bin/python rlinf/utils/ckpt_convertor/convert_xense_openpi_jax_to_pytorch.py \
        /path/to/jax_checkpoint \
        pi05_bi_flexiv \
        --inspect-only
...
def _find_norm_stats(checkpoint_dir: str, asset_id: str) -> pathlib.Path | None:
    """Locate ``norm_stats.json`` next to the checkpoint or in its assets tree."""
    checkpoint = pathlib.Path(checkpoint_dir)
    bases = (checkpoint / "assets", checkpoint.parent / "assets")
    candidates: list[pathlib.Path] = []
    for base in bases:
        if asset_id:
            candidates.append(base / asset_id / "norm_stats.json")
        candidates.append(base / "norm_stats.json")
...
def _copy_norm_stats(
    checkpoint_dir: str,
    norm_stats: str | pathlib.Path | None,
    asset_id: str,
    output_path: str,
) -> None:
    """Copy the norm-stats file into the xense-openpi assets layout."""
    if norm_stats:
        source = pathlib.Path(norm_stats)
    else:
        source = _find_norm_stats(checkpoint_dir, asset_id)
    if source is None:
        print("Warning: norm_stats.json not found; skipping norm stats copy.")
        return
    if not source.is_file():
        raise FileNotFoundError(f"norm_stats.json not found: {source}")
    dest = pathlib.Path(output_path) / "assets" / asset_id / "norm_stats.json"
    dest.parent.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(source, dest)
    print(f"Wrote norm stats: {dest}")
...
def convert_pi0_checkpoint(
    ...
    norm_stats: str | pathlib.Path | None = None,
    asset_id: str = "bi_flexiv",
    copy_norm_stats: bool = True,
):
    """
    ...
    Args:
        ...
        norm_stats: Optional path to the input norm_stats.json file
        asset_id: xense-openpi asset id used for the norm-stats directory
        copy_norm_stats: Whether to copy norm_stats.json into the output
    """
    ...
    # Copy the norm-stats asset verbatim into the xense-openpi layout.
    if copy_norm_stats:
        _copy_norm_stats(checkpoint_dir, norm_stats, asset_id, output_path)
...
def main(
    ...
    *,
    ...
    Args:
        ...
        config_name: RLinf OpenPI config name used to resolve the model architecture
        ...
        norm_stats: Optional path to the input norm_stats.json file
        asset_id: Optional xense-openpi asset id override
        copy_norm_stats: Whether to copy norm_stats.json into the output
    """
    from rlinf.models.embodiment.openpi.dataconfig import get_openpi_config

    train_config = get_openpi_config(config_name)
    ...
    if inspect_only:
        ...
        if resolved_asset_id is None:
            resolved_asset_id = getattr(
                getattr(train_config.data, "assets", None), "asset_id", None
            )
        if resolved_asset_id is None:
            resolved_asset_id = "bi_flexiv"
        convert_pi0_checkpoint(
            ...
            copy_norm_stats=copy_norm_stats,
        )
